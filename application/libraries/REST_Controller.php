...
abstract class REST_Controller extends CI_Controller
{
    ...
    protected function detectApiKey()
    {
        ...
        $gitlab_key_variable = $this->config->item('gitlab_key_name');

        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));
        $alt_key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $gitlab_key_variable));
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            return true;
            /* The else if below is customized to check for a gitlab webhook key */
        } elseif (($key = isset($this->_args[$gitlab_key_variable]) ? $this->_args[$gitlab_key_variable] : $this->input->server($alt_key_name))) {
            if (! ($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->get($this->config->item('rest_keys_table'))->row())) {
                return false;
            }
    ...
    protected function _check_access()
    {
        ...
        if (!empty($accessRow) && empty($accessRow['method'])) {
            log_message('debug', 'Controller: ' . $controller . ' Method: ' . $method);
            return $this->rest->db
            ->where('key', $this->rest->key)
